...
class Database(context: Context) :
    ...
    companion object {
        ...
        private const val DATABASE_VERSION = 6 // Increased version due to schema update
    ...
    fun getAllExpenses(username: String): List<Expense> {
        ...
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_EXPENSES WHERE $COLUMN_USERNAME = ?", arrayOf(username))
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                expenses.add(Expense(id, date, startTime, endTime, description, category, photoPath, amount))
    ...
    fun getExpensesByDateRange(username: String, startDate: String, endDate: String): List<Expense> {
        val expenses = mutableListOf<Expense>()
        val db = this.readableDatabase
    ...
    """.trimIndent()
        ...
        val cursor = db.rawQuery(query, arrayOf(username, startDate, endDate))
        if (cursor.moveToFirst()) {
            do {
                val id = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_EXPENSE_ID))
                val description = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DESCRIPTION))
    ...
    fun getCategoryTotals(username: String, startDate: String, endDate: String): List<ItemSummaryData> {
        ...
        FROM $TABLE_EXPENSES
        WHERE $COLUMN_USERNAME = ?
          AND date($COLUMN_DATE) BETWEEN date(?) AND date(?)
    ...
    fun getCategoryTotals(username: String): List<ItemSummaryData> {
        val summaryList = mutableListOf<ItemSummaryData>()
        val db = this.readableDatabase
    ...
    fun insertBudgetGoal(username: String, month: String, minGoal: Double, maxGoal: Double): Boolean {
        val db = this.writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_USERNAME, username)
    ...
    fun getBudgetGoalsForMonth(username: String, month: String): List<BudgetGoal> {
        val budgetGoals = mutableListOf<BudgetGoal>()
        val db = this.readableDatabase
        val cursor = db.rawQuery(
            "SELECT * FROM $TABLE_BUDGET_GOALS WHERE $COLUMN_MONTH = ? AND $COLUMN_USERNAME = ?",
            arrayOf(month, username)
        )
