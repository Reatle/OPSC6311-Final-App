...
class ViewSummary : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val summaryList = db.getCategoryTotals(username)
    ...
    private fun loadFilteredSummary() {
        if (startDate != null && endDate != null) {
            val filteredSummaryList = db.getCategoryTotals(username, startDate!!, endDate!!)
    ...
    private fun loadFilteredSummaryForMonth(month: String) {
        if (month.isNotEmpty()) {
            val budgetGoalList = db.getBudgetGoalsForMonth(username, month)
